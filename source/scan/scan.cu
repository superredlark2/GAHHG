...
void myscan(int* dev_output, int* dev_input, int length, bool bcao)
{
	if (length > ELEMENTS_PER_BLOCK) {
		scanLargeDeviceArray(dev_output, dev_input, length, bcao);
	}
	else {
		scanSmallDeviceArray(dev_output, dev_input, length, bcao);
	}
}
